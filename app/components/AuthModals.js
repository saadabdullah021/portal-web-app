...
const AuthModals = () => {
  ...
  // Login states
  const [loginPhoneData, setLoginPhoneData] = useState({ countryCode: "+966", phoneNumber: "" });
  ...
  const handleSignupPhoneChange = (data) => {
    setSignupPhoneData(data);
  };

  const handleSignupOtpSubmit = async (otp) => {
    const otpError = validateOtp(otp);
    if (otpError) {
      setSignupFieldErrors({ otp: otpError });
      return;
    }

    const fullPhone = signupPhoneData.countryCode + signupPhoneData.phoneNumber;
    const countryCode = signupPhoneData.countryCode.replace('+', '');
    const phoneNumber = signupPhoneData.phoneNumber;
  ...
  const handleSignupCloseOtp = () => {
    setShowSignupOtpInput(false);
  };

  const handleSignupSubmit = async () => {
    const phoneError = validatePhoneNumber(signupPhoneData.phoneNumber);
    if (phoneError) {
      setSignupFieldErrors({ phone: phoneError });
      return;
    }
    ...
    try {
      ...
      if(response.status === 200 && response.data.success){
        ...
        setSignupError("Failed to verify phone number. Please try again.");
      }
    } catch (error) {
  ...
  const handleSignupIdentityConfirmationContinue = async (method) => {
    const fullPhone = signupPhoneData.countryCode + signupPhoneData.phoneNumber;
  ...
  const handleSignupCloseIdentityConfirmation = () => {
    setShowIdentityConfirmation(false);
    handleSignupClosePopup();
  };
  ...
  const handleLoginPhoneChange = (data) => {
    setLoginPhoneData(data);
  };

  const handleLoginPhoneSubmit = async () => {
    const phoneError = validatePhoneNumber(loginPhoneData.phoneNumber);
    if (phoneError) {
      setLoginFieldErrors({ phone: phoneError });
      return;
    }

    const fullPhone = loginPhoneData.countryCode + loginPhoneData.phoneNumber;
  ...
  const handleLoginOtpSubmit = async (otp) => {
    const otpError = validateOtp(otp);
    if (otpError) {
      setLoginFieldErrors({ otp: otpError });
      return;
    }

    const fullPhone = loginPhoneData.countryCode + loginPhoneData.phoneNumber;
    const countryCode = loginPhoneData.countryCode.replace('+', '');
    const phoneNumber = loginPhoneData.phoneNumber;
    ...
    <>
      ...
      {popups.signup && (
        <div
          ...
          onClick={handleSignupClosePopup}
        >
          <div
      ...
      {showIdentityConfirmation && (
        <IdentityConfirmation
          phoneNumber={signupPhoneData.countryCode + signupPhoneData.phoneNumber}
          email={signupPhoneData.email}
          onContinue={handleSignupIdentityConfirmationContinue}
          onClose={handleSignupCloseIdentityConfirmation}
        />
      )}
      ...
      {showSignupOtpInput && (
        <OtpInput
          ...
          onClose={handleSignupCloseOtp}
          error={!!signupFieldErrors.otp || !!signupError}
          errorMessage={signupFieldErrors.otp || signupError}
      ...
      {showLoginOtpInput && (
        <OtpInput
          phoneNumber={loginPhoneData.countryCode + loginPhoneData.phoneNumber}
          onSubmit={handleLoginOtpSubmit}
          onResend={handleLoginOtpResend}
          onClose={handleLoginCloseOtp}
          error={!!loginFieldErrors.otp || !!loginError}
          errorMessage={loginFieldErrors.otp || loginError}
